...
def define_data_input_distributed(model, queue_batch=None):
...
#  coord, volname = inputs.load_patch_coordinates(FLAGS.train_coords)
  ...
  # Creates a Dataset that reads coordinate files and lets each worker read a 
  # unique subset when running distributed training
  d = tf.data.TFRecordDataset(FLAGS.train_coords, compression_type='GZIP')
  d = d.shard(hvd.size(), hvd.rank())
  d = d.map(parser_fn)
  iterator = d.make_one_shot_iterator()

  # Get a single coordinate and volume name
